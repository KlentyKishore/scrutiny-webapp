...
const User = {
  ...
  getMyQuestions: function (req, res) {
    db.Question.find({ userId: req.user._id })
      .then((questions_obj) => {
  ...
  getMyQuestion: function (req, res) {
    const condition = { _id: req.params.questionId}
    if(req.user.role !== 'admin'){
      condition["editAccess"]= { $in: [req.user.email] }
    }
    db.Question.findOne(condition)
      .then((question_obj) => {
        if (question_obj) {
          res.json({ status: "sucess", question: question_obj });
        } else {
          res.json({ status: "failed", msg: "You Don't have access to edit" });
        }
      })
      .catch((err) => {
        Util.logError(err.msg, err);
  ...
  likeMyQuestion: function (req, res) {
    if (req.params.questionId) {
      db.Question.findOneAndUpdate(
        { _id: req.params.questionId, likes: { $nin: [String(req.user._id)] } },
  ...
  createFolder: function(req, res) {
     if (req.body.folderName && req.body.isParent){
      db.ImpactFolder.create({
        ...
        parentId: req.body.parentId,
          ...
          res.json({
            status: "sucess",
            msg: "sucessfully added your impact region",
          });
        })
        .catch((err) => {
          Util.logError(err.msg, err);
  ...
  getParentFolder: function(req, res) {
      return    db.ImpactFolder.find({
          isParent:true
      }).then((data)=>{
  ...
  addMyQuestion: function (req, res) {
    if (req.body.feature && req.body.impactRegion) {
      db.Question.create({
        userId: req.user._id,
        feature: req.body.feature,
        feature: req.body.feature,
        notes: req.body.notes,
        impactRegion: req.body.impactRegion,
        owner: req.user.name,
        parentFolderId : req.body.parentFolderId,
        subFolderId : req.body.subFolderId
  ...
  getQuestions: function (req, res) {
    if (req.params.subFolderID) {
      db.Question.find({ subFolderId: req.params.subFolderID })
        .then((questions_obj) => {
          questions_obj.forEach((question) => {
            question._doc.user = { name: req.user.name };
          });
  ...
  updateMyQuestion: function (req, res) {
    if (
      ...
      req.body.question.feature &&
      req.body.question.impactRegion &&
      req.body.question.impactRegion
    ) {
      db.Question.findOneAndUpdate(
        { _id: req.body.question_id, userId: req.body.question.userId },
        {
          feature: req.body.question.feature,
          impactRegion: req.body.question.impactRegion,
          notes: req.body.question.notes,
        }
      )
        .then((question_obj) => {
          res.json({
            status: "sucess",
            msg: "sucessfully updated your question",
          });
        })
        .catch((err) => {
          res.json({ status: "failed", msg: "Something went wrong" });
  ...
  deleteMyQuestion: function (req, res) {
    if (req.params.questionId) {
      db.Question.findOneAndRemove({ _id: req.params.questionId })
        .then((question_obj) => {
          res.json({
            status: "sucess",
            msg: "sucessfully deleted your question",
          });
          //also remove all the answer
          db.Answer.deleteMany({ questionId: req.params.questionId }).then(
            (answer_obj) => {}
          );
        })
        .catch((err) => {
  ...
  getMyAnswers: function (req, res) {
    db.Answer.find({ userId: req.user._id })
      .then((answers_obj) => {
  ...
  addMyAnswer: function (req, res) {
    if (req.body.answer && req.body.feature_id) {
      db.Answer.create({
        userId: req.user._id,
        questionId: req.body.feature_id,
        ...
        .then((answer_obj) => {
          if (answer_obj) {
            res.json({
              status: "sucess",
              answer: { ...answer_obj._doc, user: { name: req.user.name } },
            });
          } else {
            res.json({
              status: "failed",
  ...
  likeMyAnswer: function (req, res) {
    if (req.params.answerId) {
      db.Answer.findOneAndUpdate(
        { _id: req.params.answerId, likes: { $nin: [String(req.user._id)] } },
        { $push: { likes: [String(req.user._id)] } }
      ).then((answer_obj) => {
        if (!answer_obj) {
          //if user already liked remove his id
  ...
  updateMyAnswer: function (req, res) {
    if (req.body.answer_id && req.body.answer) {
      db.Answer.findOneAndUpdate(
        { _id: req.body.answer_id, userId: req.user._id },
        { answer: req.body.answer }
      )
        .then((answer_obj) => {
          if (answer_obj) {
            res.json({
              status: "sucess",
              msg: "sucessfully updated your answer",
            });
          } else {
            res.json({ status: "failed", msg: "Something went wrong" });
          }
  ...
  deleteMyAnswer: function (req, res) {
    if (req.params.answerId) {
      db.Answer.findOneAndRemove({ _id: req.params.answerId })
        .then((answer_obj) => {
          res.json({
            status: "sucess",
            msg: "sucessfully deleted your answer",
          });
        })
        .catch((err) => {
          Util.logError(err.msg, err);
  ...
  requestAcess: function (req, res) {
    console.log(req.params.questionId, "/users");
    if (req.params.questionId) {
      db.EditAccess.create({
        email: req.user.email,
        questionId: req.params.questionId,
      }).then(() => {
        res.json({ status: "success", msg: "Wait for admin to accept it" });
      });
    }
  },
};
